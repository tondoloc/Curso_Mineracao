...
# Corpus preliminar : volecao de textos para cada doc
# comando do pacote tm para mineracao de textos
corpusJE <- Corpus(VectorSource(bookJE)) %>%
            tm_map(removePunctuation) %>% # remove pontuacao
            tm_map(removeNumbers) %>% # remove numeros
            tm_map(content_transformer(tolower)) %>% # transforma em minuculas
            tm_map(removeWords, stopwords("english")) %>% #remove palavras irrelevantes "i and the" 
            tm_map(stripWhitespace) %>% # remove espacos em branco extras
            tm_map(stemDocument) # analisa pe stop, stopped, stopping e deixa só a raiz
...
# objeto bronte e mescla as tabelas combinando palavras e classificando
# por palavras criar algumas variaveis dProp é a dif de proporcoes e 
# converter em valores abs para encontrar os distintos que são mais comuns
# entre os dois e classificados em valores descrescentes com base
# na dif abs e renomear as colunas
